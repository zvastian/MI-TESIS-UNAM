...
PROMPT_PATH = BASE_DIR / "prompts" / "research_questions.md"
PAYLOAD_PATH = BASE_DIR / "payloads" / "ai_questions_payload.json"
OUTPUT_PATH = BASE_DIR / "outputs" / "ai_questions_groq_20b.json"
...
def validate_structure(parsed: dict) -> tuple[bool, str]:
    rq = parsed.get("research_questions")

    if not isinstance(rq, dict):
        return False, "missing or invalid 'research_questions'"
    ...
    if len(questions) != 4:
        return False, f"expected 4 questions, got {len(questions)}"
    ...
    for i, q in enumerate(questions):
        ...
        methodological_angle = str(q.get("methodological_angle", "")).strip()
        why_it_matters = str(q.get("why_it_matters", "")).strip()

        if not qtype:
            return False, f"empty type in question {i}"

        if not question:
            return False, f"empty question text in question {i}"

        if not methodological_angle:
            return False, f"empty methodological_angle in question {i}"

        if not why_it_matters:
            return False, f"empty why_it_matters in question {i}"

        if qtype in seen_types:
            return False, f"duplicate type: '{qtype}'"

        seen_types.add(qtype)

        word_count = len(question.split())
        if word_count > 24:
            return False, f"question {i} too long ({word_count} words, max 24)"

    return True, "ok"


def validate(parsed: dict) -> tuple[bool, str]:
    ok, msg = validate_structure(parsed)
...
def build_repair_message(
    ...
    return (
        ...
        "Debe contener research_questions.title y exactamente 4 preguntas. "
        "Cada pregunta debe incluir type, question, methodological_angle y why_it_matters. "
        "No agregues explicación, markdown ni texto externo.\n\n"
...
def main():
    ...
    original_user_message = (
        "Formula preguntas de investigación para esta tesis. "
    ...
    for attempt in range(1, MAX_RETRY_ATTEMPTS + 2):
        ...
        try:
            ...
            valid, validation_msg = validate(parsed)
        except Exception as exc:
            parsed = {
