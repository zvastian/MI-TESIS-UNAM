...
(() => {
  ...
  async function loadRankingData() {
    ...
    try {
      const response = await fetch(`/api/workshop/tools/ranking?${params.toString()}`);
      if (!response.ok) {
        const detail = await response.text();
        throw new Error(detail || `HTTP ${response.status}`);
      }
  ...
  function ensureInitialBubblesAdvisor() {
    // Auto-default de burbujas desactivado.
    return;
  }
...
/* WCT DEFAULT HEATMAP START */
(() => {
  function ensureDefaultWorkshopHeatmap() {
    if (document.body?.dataset?.tab !== "taller") return;

    const heatmapBtn = document.querySelector('[data-workshop-tool="heatmap"]');
    if (!heatmapBtn) return;

    const activeTool = document.querySelector("[data-workshop-tool].is-active")?.dataset?.workshopTool;
    const hasHeatmapShell = !!document.querySelector("#heatmapMatrixSelect");
    ...
    if (!hasHeatmapShell && typeof window.renderWorkshopHeatmap === "function") {
      window.renderWorkshopHeatmap();
    }
  }

  const bodyObserver = new MutationObserver(() => {
    requestAnimationFrame(() => {
      setTimeout(ensureDefaultWorkshopHeatmap, 40);
    });
  });
  ...
  if (document.body) {
    bodyObserver.observe(document.body, {
      attributes: true,
      attributeFilter: ["data-tab", "class"],
    });
  }

  window.ensureDefaultWorkshopHeatmap = ensureDefaultWorkshopHeatmap;

  requestAnimationFrame(() => {
    setTimeout(ensureDefaultWorkshopHeatmap, 120);
  });
})();
/* WCT DEFAULT HEATMAP END */
